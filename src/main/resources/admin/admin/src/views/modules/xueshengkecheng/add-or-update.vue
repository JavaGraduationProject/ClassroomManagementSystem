<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="是否报读" prop="shifoubaodu">
          <el-select v-model="ruleForm.shifoubaodu" placeholder="请选择是否报读">
            <el-option
                v-for="(item,index) in shifoubaoduOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="是否报读" prop="shifoubaodu">
	      <el-input v-model="ruleForm.shifoubaodu"
                placeholder="是否报读" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="报读时间" prop="baodushijian">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.baodushijian" 
                type="datetime"
                placeholder="报读时间">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.baodushijian" label="报读时间" prop="baodushijian">
              <el-input v-model="ruleForm.baodushijian" 
                placeholder="报读时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  created() {
	this.ruleForm.baodushijian = this.getCurDateTime()
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.kechengmingcheng = true;
          ...
          if(o=='shifoubaodu'){
            this.ruleForm.shifoubaodu = obj[o];
	    this.ro.shifoubaodu = true;
            continue;
          }
          if(o=='baodushijian'){
            this.ruleForm.baodushijian = obj[o];
	    this.ro.baodushijian = true;
            continue;
          }
      ...
      this.$http({
		...
		if(json.xingming!=''&&json.xingming){
          ...
          this.$message.error(data.msg);
            ...
            this.shifoubaoduOptions = "是,否".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `xueshengkecheng/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `xueshengkecheng/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.xueshengkechengCrossAddOrUpdateFlag = false;
                  this.parent.search();
