<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="课堂表现" prop="ketangbiaoxian">
          <el-select v-model="ruleForm.ketangbiaoxian" placeholder="请选择课堂表现">
            <el-option
                v-for="(item,index) in ketangbiaoxianOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="课堂表现" prop="ketangbiaoxian">
	      <el-input v-model="ruleForm.ketangbiaoxian"
                placeholder="课堂表现" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="学习态度" prop="xuexitaidu">
          <el-select v-model="ruleForm.xuexitaidu" placeholder="请选择学习态度">
            <el-option
                v-for="(item,index) in xuexitaiduOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="学习态度" prop="xuexitaidu">
	      <el-input v-model="ruleForm.xuexitaidu"
                placeholder="学习态度" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.xingming = true;
          ...
          if(o=='ketangbiaoxian'){
            this.ruleForm.ketangbiaoxian = obj[o];
	    this.ro.ketangbiaoxian = true;
            continue;
          }
          if(o=='xuexitaidu'){
            this.ruleForm.xuexitaidu = obj[o];
	    this.ro.xuexitaidu = true;
            continue;
          }
          if(o=='jiaoshipingyu'){
      ...
      this.$http({
		...
		if(json.xingming!=''&&json.xingming){
          ...
          this.$message.error(data.msg);
            ...
            this.xuexitaiduOptions = "★,★★,★★★,★★★★,★★★★★".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `ketangbiaoxian/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `ketangbiaoxian/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.ketangbiaoxianCrossAddOrUpdateFlag = false;
                  this.parent.search();
